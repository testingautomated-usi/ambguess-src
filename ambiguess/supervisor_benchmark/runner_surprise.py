"""Various variants of surprise adequacy. Based on Weiss et. al., ISSTA 2022"""
...
from supervisor_benchmark.model_architectures import Activations
from supervisor_benchmark.stable_kde import StableGaussianKDE
...
def run(data: TestSets,
    ...
    # Get training data
    train_x, _ = data.mixed_ambiguous_training_data()
    sa_layers = architecture.get_sa_layers()
    train_activations, train_pred = architecture.get_activations_and_pred(model, sa_layers, train_x)
    train_activations = _flatten_layers(train_activations)

    # shuffle training activations and predictions
    train_activations, train_pred = sklearn.utils.shuffle(train_activations, train_pred, random_state=run_id)

    # We use one third of the dsa samples
    samples_for_dsa = math.floor(train_activations.shape[0] / 3)
    sas = {
        'DSA': DSA(train_activations[:samples_for_dsa], train_pred[:samples_for_dsa], badge_size=30),
        'LSA': MultiModalSA.build_by_class(train_activations, train_pred, lambda x, _: LSA(x)),
        'MDSA': MultiModalSA.build_by_class(train_activations, train_pred, lambda x, _: MDSA(x)),
    }

    # Run for nominal data (which is also used later)
    nom_x, nom_y_true = data.nominal_test_data()
    nom_activations, nom_pred = architecture.get_activations_and_pred(model, sa_layers, nom_x)
    nom_activations = _flatten_layers(nom_activations)

    evals = dict()
    for sa_name, sa in sas.items():
        nom_surprise = sa(nom_activations, nom_pred)
        evals[sa_name] = Evaluation(nom_surprise)

    for test_set_name, test_set_x_y in data.non_nominal_as_dict(run_id=run_id).items():
        ts_activations, ts_pred = architecture.get_activations_and_pred(model, sa_layers, test_set_x_y[0])
        ts_activations = _flatten_layers(ts_activations)
        for sa_name, sa in sas.items():
            surprise = sa(ts_activations, ts_pred)
            auc_roc = evals[sa_name].auc_roc(surprise)
...
def _subsample_array(subsampling: Union[int, float],
                     array: np.ndarray,
                     seed: int) -> np.ndarray:
...
def _subsample_arrays(subsampling: Union[int, float],
    ...
    if subsampling == 1.0:
        ...
        raise ValueError("subsampling must be a float between 0 and 1"
...
def _class_predictions(predictions: Predictions, num_classes: int = None) -> np.ndarray:
    ...
    assert predictions.ndim == 1, "Class predictions must be one-dimensional. " \
                                  "If your predictions are one_hot encoded, use " \
                                  "eg `np.argmax(softmax_outputs, axis=1)`"

    if not predictions.dtype == np.int:
        np.testing.assert_almost_equal(predictions, predictions.astype(np.int), decimal=5,
...
class MultiModalSA(SA):
    ...
    @staticmethod
    def _calculate_surprise_for_modal(sa: SA,
                                      activations: Activations,
                                      predictions: Optional[Predictions],
    ...
    def _get_sa_for_modal_id(self, modal_id: int) -> SA:
        ...
        try:
            ...
            raise ValueError(f"No modal found for modal id {modal_id}. Check your discriminator")
    ...
    def __call__(self,
        ...
        with ThreadPoolExecutor(max_workers=workers) as executor:
            for modal_id in modals_in_this_set:
                ...
                p = None if predictions is None else predictions[discriminator_idxs == modal_id]
                workers = int(np.ceil(remaining_workers / (len(modals_in_this_set))))
                remaining_workers -= workers
                f = executor.submit(self._calculate_surprise_for_modal, sa, a, p, workers)
                futures.append(f)

        per_modal_values = [f.result() for f in futures]
        res = np.full(fill_value=-np.inf, shape=discriminator_idxs.shape, dtype=per_modal_values[0].dtype)
...
class MDSA(SA):
    ...
    def __init__(self,
        ...
        super().__init__()
        activations = _flatten_layers(activations)
        self.covariance_matrix: EmpiricalCovariance = sklearn.covariance.EmpiricalCovariance()
...
class LSA(SA):
    ...
    def __init__(self,
        ...
        if var_threshold is not None and var_threshold > 0:
            activations = _flatten_layers(activations)
            self.removed_neurons = np.where(np.var(activations, axis=0) < var_threshold)[0]

        # Figure out which neurons to remove, as they have low variance
        if max_features is not None:
            if max_features < 1:
                num_features = min(max_features * activations.shape[1], activations.shape[1])
            else:
    ...
    def _create_gaussian_kde(self,
                             activations: np.ndarray) -> gaussian_kde:
        cleaned_activations = self._remove_unused_columns(activations)
        if cleaned_activations.shape[1] == 0:
            warnings.warn((f"The passed min_var threshold {self.min_var_threshold} and/or"
                           f"the automatic removal of numerically unstable features"
                           f"led to the removal of all ATs. This instance of LSA will"
                           f"thus always return density 0"), UserWarning)
            self.kde = None
        else:
            try:
                return StableGaussianKDE(cleaned_activations.transpose())
            except (np.linalg.LinAlgError, ValueError) as e:
                if ("-th leading minor of the array is not positive definite" in str(e) or
                        "numerical imprecision in covariance matrix" in str(e)):
                    problematic_row = int(re.findall("\\d*", str(e))[0]) - 1
                    original_indexes = np.delete(np.arange(activations.shape[1]), self.removed_neurons)
                    problematic_index = original_indexes[problematic_row]

                    warnings.warn(f"Dropping AT {problematic_index}, as leading to numerical error.", UserWarning, 1)
...
class DSA(SA):
    ...
    def __call__(self,
        ...
        with ThreadPoolExecutor(max_workers=5) as executor:
            ...
            future_to_url = {executor.submit(task, params[0], params[1]): params for params in task_inputs}
            for future in tqdm.tqdm(as_completed(future_to_url),
    ...
    def _dsa_distances(self, sample_ats: np.ndarray, label: int) -> \
        ...
        other_classes_indexes = np.ones(shape=self.train_activations.shape[0], dtype=bool)
        other_classes_indexes[self.class_matrix[label]] = 0
        train_matches_other_classes = self.train_activations[other_classes_indexes]
        dist_b = self._get_closest_ats(ats_a, train_matches_other_classes, calc_ats=False)
    ...
    def _check_memory_warning(self,
        ...
        if num_threads is None:
            ...
            if cpu_count is None:
                warnings.warn("Could not determine the number of CPUs. ,"
                              "Hence, cannot estimate DSA memory use",
                              RuntimeWarning)
                return
            if version.Version(f"{sys.version_info.major}.{sys.version_info.minor}") \
                    >= version.Version("3.8"):
...
def _memory_warning(required_memory: int):
    ...
    if available_memory is not None and required_memory > available_memory / 2:
        ...
        warnings.warn(f"Expected peak memory use for DSA is higher than {human_readable}, "
                      f"which exceeds 50% available memory.", UserWarning, 2)
